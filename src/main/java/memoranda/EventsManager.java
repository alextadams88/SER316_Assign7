			...
			if (date.inPeriod(ev.getStartDate(), ev.getEndDate())) {
				if (ev.getRepeat() == REPEAT_DAILY) {
					...
					if ((n - ns) % ev.getPeriod() == 0)
						v.add(ev);
				} else if ((ev.getRepeat() == REPEAT_WEEKLY && date.getCalendar().get(Calendar.DAY_OF_WEEK) == ev.getPeriod()) || (ev.getRepeat() == REPEAT_MONTHLY && date.getCalendar().get(Calendar.DAY_OF_MONTH) == ev.getPeriod())) {
						v.add(ev);
