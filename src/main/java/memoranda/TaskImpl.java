...
*/
	...
	public Collection getSubTasks() {
		Elements subTasks = _element.getChildElements("task");
            return convertToTaskObjects(subTasks, _tl);
	}

	//TASK 2-2 SMELL BETWEEN CLASSES
	//making this public static so it can be called by TaskListImpl instead of duplicating the code
	public static Collection convertToTaskObjects(Elements tasks, ITaskList list) {
        Vector v = new Vector();
        for (int i = 0; i < tasks.size(); i++) {
            ITask t = new TaskImpl(tasks.get(i), list);
            v.add(t);
        }
        return v;
    }
	...
	//TASK 2-2 SMELL BETWEEN CLASSES
	//moving this code from TaskListImpl because this should be a TaskImpl responsibility
    public boolean isActive(CalendarDate date) {
        if ((this.getStatus(date) == ITask.ACTIVE) || (this.getStatus(date) == ITask.DEADLINE) || (this.getStatus(date) == ITask.FAILED)) {
            return true;
        }
        else {
            return false;
